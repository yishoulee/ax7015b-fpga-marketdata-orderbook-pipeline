...
# AXI FIFO MM-S smoke test (TX write -> expect RX via whatever BD loopback exists)
# Uses known-good XSCT memmap syntax for PL addresses.
# Avoids reading 0x24 (RLR) because it can trigger AHB AP transaction errors.
...
proc memmap_set_rw {base size_hex} {
    # Clear first (ignore if not present)
...
# AXI FIFO MM-S register offsets (avoid 0x24)
set ISR  0x00
set IER  0x04
set TDFR 0x08
set TDFV 0x0C
...
banner "probe MYIP + FIFO (safe regs)"
...
if {$got_words < $want_words} {
    error [format {Timeout: RDFO never reached %d words (stuck at %d)} $want_words $got_words]
}
